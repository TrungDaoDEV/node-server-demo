...
io.on('connection', function (socket) {	//'connection' (1) này khác gì với 'connection' (2)
    ...
    //Gửi đi lệnh 'welcome' với một tham số là một biến JSON. 
    //Trong biến JSON này có một tham số và tham số đó tên là message. 
    //Kiểu dữ liệu của tham số là một chuối.
    socket.emit('welcome', {
        message: 'Connected !!!!'
    });
    //Khi lắng nghe được lệnh "connection" với một tham số, và chúng ta đặt tên tham số là message. 
    //Mình thích gì thì mình đặt thôi.
    //'connection' (2)
