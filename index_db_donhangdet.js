...
io.on("connection", function (socket) {
    ...
    socket.on("TTMD", function (data) {
        ...
        var datasql2 = { idRJ: null, May: data.M, Ngay: date, SL: data.SL, Trangthai: data.TT, Nguon: data.P, idMD: 4, TG_OFF: data.TOF, TG_ON: data.TON };
...
function handleDisconnect() {
    ...
    db.on('error', function (err) {
        ...
        } else if (err.code === 'App crashed') {                                      // connnection idle timeout (the wait_timeout
...
app.get('/taisao', (req, res) => {
    ...
    db.query(sql, (err, result) => {
        ...
        console.log("get    /tai sao lai jo load đuoc");
        res.send(result);//goi kq cho react native
    })
})
